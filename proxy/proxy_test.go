...
func TestGetCommandURL(t *testing.T) {
	tests := []struct {
		...
		{
			name:            "no command endpoint set",
			serverURL:       "https://example.com/sse",
			commandEndpoint: "",
			expectedURL:     "https://example.com/message",
		},
		{
			name:            "relative command endpoint",
			serverURL:       "https://example.com/sse",
			commandEndpoint: "/api/command",
			expectedURL:     "https://example.com/api/command",
		},
		{
			name:            "absolute command endpoint",
			serverURL:       "https://example.com/sse",
			commandEndpoint: "https://api.example.com/command",
			expectedURL:     "https://api.example.com/command",
		},
		{
			name:            "server URL with port",
			serverURL:       "https://example.com:8080/sse",
			commandEndpoint: "",
			expectedURL:     "https://example.com:8080/message",
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			proxy, err := NewProxy(tt.serverURL, 3334, map[string]string{}, "test-hash")
			if err != nil {
...
func TestSetAndGetCommandEndpoint(t *testing.T) {
	...
	if endpoint := proxy.GetCommandEndpoint(); endpoint != "" {
		t.Errorf("Expected empty endpoint, got %s", endpoint)
	}

	// Set endpoint
	testEndpoint := "/api/command"
	proxy.SetCommandEndpoint(testEndpoint)

	// Get the set endpoint
	if endpoint := proxy.GetCommandEndpoint(); endpoint != testEndpoint {
